...
- [10. Guia de Defesa: Domínio & Modificações Rápidas](#10-guia-de-defesa-domínio--modificações-rápidas)
...
> Observação: no código há _ifdefs_ para Windows (`_lseeki64`, `_read/_write`, `_fileno`) e POSIX (`lseek`, `read`, `write`).
...
Use o gerador em `tests/` (histórias) ou seu próprio texto duplicado; execute buscas com `--count-only` para medir performance.

---

## 10. Guia de Defesa: Domínio & Modificações Rápidas
...
- **Mods rápidas** (se pedirem ao vivo):  
  1) `--case-insensitive` (ASCII) normalizando bytes/padrão no KMP;  
  2) `--first-n=N` para interromper após N matches;  
  3) imprimir **Peak Working Set** além do atual.
